...
router.get('/debug-payone-exhaustive', async (req, res) => {
    const logger = require('../utils/logger');

    // We will test 6 scenarios to find the working combination
    const scenarios = [
        { id: 'S1', curr: '682', amt: '5', lang: 'en', desc: 'Baseline_682_Int_EN' }, // Should work based on previous simple debug
        { id: 'S2', curr: '682', amt: '5', lang: 'ar', desc: 'Test_682_Int_AR' },     // Test if AR breaks it
        { id: 'S3', curr: '682', amt: '5.00', lang: 'ar', desc: 'Test_682_Dec_AR' },    // Test Decimal with 682
        { id: 'S4', curr: 'SAR', amt: '5', lang: 'ar', desc: 'Test_SAR_Int_AR' },     // Test Alpha SAR with Integer
        { id: 'S5', curr: 'SAR', amt: '500', lang: 'ar', desc: 'Test_SAR_Minor_AR' },   // Test Alpha SAR with Minor Units
        { id: 'S6', curr: 'SAR', amt: '5.00', lang: 'ar', desc: 'Test_SAR_Dec_AR' },    // Test Alpha SAR with Decimal
    ];

    const results = [];
    const baseUrl = process.env.PAYONE_BASE_URL || 'https://smartlinkdb-test.payone.io/URL2PayAdminWeb/rest/InvoicesService';
    const apiUrl = baseUrl.endsWith('/') ? baseUrl + 'createInvoice' : baseUrl + '/createInvoice';

    const merchantID = process.env.PAYONE_MERCHANT_ID;
    const authToken = (process.env.PAYONE_AUTH_TOKEN || '').replace(/\s+/g, '');

    for (const scen of scenarios) {
        const uniqueID = 'PRB-' + scen.id + '-' + Date.now();
        ...
        const payload = {
            ...
            invoicesDetails: [{
                ...
                amount: scen.amt,
                currency: scen.curr,
                paymentDescription: `Debug ${scen.desc}`,
                customerID: 'DbgUser',
                customerEmailAddress: 'dbg@test.com',
                language: scen.lang,
                expiryperiod: '1D',
        ...
        try {
            const response = await fetch(apiUrl, {
                ...
                body: params,
                headers: {
                    'User-Agent': 'Mozilla/5.0 (Vercel; Probe)'
                }
            });
            ...
            let status = 'FAILED';
            if (json) {
                if (json.invoicesDetails && json.invoicesDetails.length > 0) status = 'SUCCESS';
                else if (json.Error === '00010') status = 'SUCCESS_DUP'; // Duplicate ID means it was accepted!
                else status = `API_ERR_${json.Error}`;
            } else {
                status = 'HTTP_ERR';
            }
            ...
            results.push({
                scenario: scen.desc,
                sent_amount: scen.amt,
                sent_currency: scen.curr,
                status: status,
                raw_response: text.substring(0, 200)
            });

        } catch (e) {
            results.push({ scenario: scen.desc, status: 'EXCEPTION', error: e.message });
        }
    }

    res.json({
        summary: "Payone Probe Results",
